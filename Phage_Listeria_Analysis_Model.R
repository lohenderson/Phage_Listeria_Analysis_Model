phage<-read.csv("Cheese_Phage_Masterfile.csv")
#phage$inoculum[phage$inoculum==0] <- mean(phage$inoculum[phage$inoculum>0],na.rm=T)
#phage$inoculum[is.na(phage$inoculum)] <- mean(phage$inoculum[phage$inoculum>0],na.rm=T)

# Change columns to appropriate data types; add log columns
phage %>%
  mutate(
    day=as.factor(day),
    ...
    log_inoculum=ifelse(inoculum>0,log10(inoculum),NA),
    log_milk_apc=log10(milk_apc+1), # Add one so zeroes aren't excluded
...
lmer(log_count ~ temperature*Phage + Phage*Strain + temperature*day + Phage*day +
       milk_age +
       log_milk_apc +
...
lmer(log_count ~ ph*Phage + Phage*Strain + ph*day + Phage*day +
       milk_age +
       log_milk_apc +
       (1|milk_batch/cheese_make/cheese_plate) +
       (1|rep),
